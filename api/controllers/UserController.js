...
module.exports = {
  create: function (req, res) {
    ...
    if (username.length <= 3) {
      return res.badRequest({ err: "Username needs to be greater than 3 characters." });
    }

    if (password.length <= 3) {
      return res.badRequest({ err: "Password needs to be greater than 3 characters." });
    }
  ...
  loginLdap: function (req, res) {
    ...
    ldapService.auth(req.param('username'), req.param('password'), function (err, user) {
      if (err) {
